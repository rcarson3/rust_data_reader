...
//The test file is in scientific notation to test functions ability to parse floating point numbers
fn load_txt_f64_large_old_test() {
    let file = String::from("grainData_LOFEM.rods");
    ...
    let _results = load_txt_old_f64(&file, &params);
...
fn test_wrapper(c: &mut Criterion) {
    c.bench(
        "load_txt_float",
        Benchmark::new("load_txt_f64_large_txt", |b| {
...
fn test_wrapper2(c: &mut Criterion) {
    c.bench(
        "load_txt_old_float",
        Benchmark::new("load_txt_old_f64_large_txt", |b| {
            b.iter(|| load_txt_f64_large_old_test())
        })
        .sample_size(10),
    );
}
