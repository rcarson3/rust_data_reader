            ...
            match &params.usecols{
                Some(x) =>{
                    results.results.extend({
                            x.iter().map(|y| f32::from_str(line_split_vec[*y].trim()).unwrap())
                        });
                }
                None =>{
                    results.results.extend({
                        line_split_vec.iter().map(|x| f32::from_str(x.trim()).unwrap())
                    });
                }
            }
        }
    ...
    while reader.read_line(&mut line).unwrap() > 0{
        ...
        if !line.starts_with(&comment){
            ...
            match &params.usecols{
                Some(x) =>{
                    results.results.extend({
                            x.iter().map(|y| f64::from_str(line_split_vec[*y].trim()).unwrap())
                        });
                }
                None =>{
                    results.results.extend({
                        line_split_vec.iter().map(|x| f64::from_str(x.trim()).unwrap())
                    });
                }
            }
        }
